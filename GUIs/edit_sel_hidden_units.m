...
if strcmp(sel_choice, 'Mean')
    ...
    if label_choice ~= 1
        ...
        hidden_sel_nontarget = mean(main_GUI_data.DL_saved_res.Hidden(Y==1, :));
        hidden_sel_target = mean(main_GUI_data.DL_saved_res.Hidden(Y==0, :));
        if sum(strcmp(fieldnames(main_GUI_data.DL_saved_res)', 'Weight4Label'))
            w = main_GUI_data.DL_saved_res.Weight4Label;
            hidden_sel = single(w(1, :) < w(2, :));
        else
...
elseif strcmp(sel_choice, 'input')
